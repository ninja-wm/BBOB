...
def f3(x=None,xopt=None,fopt=None):
    ...
    z=fTosz(z)
    z=fTasy(z,0.2)
    z=(fjianAlpha(10,z.shape[-1])@z.T).T      #(n,d)@(d,d)
...
def f6(x=None,xopt=None,fopt=None,Q=None):
    z=(Q@fjianAlpha(10,x.shape[-1])@Q@(x-xopt).T).T
...
def f7(x=None,xopt=None,fopt=None,Q=None,R=None):
    z_hat=(fjianAlpha(10,x.shape[-1])@R@(x-xopt).T).T
...
def f15(x=None,xopt=None,fopt=None,Q=None,R=None):
    ...
    z=fTasy(z,0.2)
    z=(Q@z.T).T
    z=(fjianAlpha(10,z.shape[-1])@z.T).T
    z=(R@z.T).T
...
def f16(x=None,xopt=None,fopt=None,Q=None,R=None):
    ...
    z=fTosz(z)
    z=(Q@z.T).T
    z=(fjianAlpha(0.01,z.shape[-1])@z.T).T
    z=(R@z.T).T
...
def f18(x=None,xopt=None,fopt=None,Q=None,R=None):
    ...
    z=fTasy(z,0.5)
    z=(Q@z.T).T
    z=(fjianAlpha(1000,z.shape[-1])@z.T).T
    s=torch.sqrt(torch.pow(z[:,:-1],2)+torch.pow(z[:,1:],2))
    item=1.0/s.shape[-1]*torch.sum(torch.sqrt(s)*(1+torch.pow(torch.sin(50*torch.pow(s,0.2)),2)),dim=-1)
    r=torch.pow(item,2)+10*fpen(x)+fopt
    return r
...
def f20(x=None,xopt=None,fopt=None):
    ...
    item=(fjianAlpha(10,zhat.shape[-1])@(zhat-2*torch.abs(xopt)).T).T
...
def f21(x=None,y=None,fopt=None,R=None,C=None):
    ...
    item=(z0.transpose(-1,-2))@(R.T)@C@R@z0  #(N,101,1,dim)@(101,dim,dim)@(dim,dim)@(n,101,dim,1)->(n,101,1,1)
...
def f23(x,xopt=None,fopt=None,Q=None,R=None):
    x=x-xopt
    x0=x.T
    z=(Q@fjianAlpha(100,x.shape[-1])@R@x0).T
    d=x.shape[-1]
...
def f24(x,xopt=None,fopt=None,Q=None,R=None):
    ...
    xhat=2*fsign(xopt)*x
    xhat1=xhat.T
    z=(Q@fjianAlpha(100,D)@R@(xhat1-miu0)).T
