...
def genC(fid,dim):
    if fid==22:
        alpha=np.random.choice([np.power(1000,float(2*i)/19) for i in range(20)],size=20,replace=False)
        ...
        alpha=np.random.choice([np.power(1000,float(2*i)/99) for i in range(100)],size=100,replace=False)
...
def getFitness(x,fun):
    ...
    if fun['fid'] in [1,2,3,4,5,8,20,]:
        ...
        r=fun['fun'](x,xopt=fun['xopt'],fopt=fun['fopt'],Q=fun['Q'],R=fun['R'])
